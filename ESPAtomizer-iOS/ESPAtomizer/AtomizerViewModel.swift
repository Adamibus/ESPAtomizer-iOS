...
/// The device's last-reported state, as parsed from BLE notifications/reads.
/// Plain value type — never persisted, so no Codable. Fields that duplicated
/// the ViewModel's own published scalars (kp/ki/kd/manual/spmode/batV) were
/// removed: they were written on every notification and read by nothing.
struct AtomizerStatus {
    var temp: Double? = nil
    var setpoint: Double = 240.0  // 240 °C — good default dab temp
...
class AtomizerViewModel: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    // 🎨 UI-CUSTOMIZE: THE app brand. Change this string to rename the product
    // everywhere in the UI (the large header title).
    ...
    // PID as one value, for persistence. The app tunes ONE loop (Auto); the old
    // 5-slot per-mode setpoint/PID arrays existed for U1/U2/Config tabs this UI
    // no longer has — the device dial owns those presets now.
    struct PID: Codable { var kp: Double; var ki: Double; var kd: Double }
    static let defaultPID = PID(kp: 10.0, ki: 0.5, kd: 50.0)
    ...
    // (togglePower / setSetpoint / setManualOutput / setMode / setPID),
    ...
    /// v4: 5-slot per-mode setpoint/PID arrays collapsed to one setpoint + one
    ///     PID (the app tunes only the Auto loop; U1/U2 live on the device dial).
    private let configVersion = 4
    ...
    func restoreConfigState() {
        if let data = UserDefaults.standard.data(forKey: configKey) {
            ...
            if let saved = try? decoder.decode(Saved.self, from: data) {
                ...
                // (Pre-v4 configs stored arrays under different keys, so the
                // scalar `setpoint`/`pid` simply decode as nil there.)
                if !needsMigration, let sp = saved.setpoint { self.setpoint = sp }
                if let pid = saved.pid { self.kp = pid.kp; self.ki = pid.ki; self.kd = pid.kd }
    ...
    func setTempUnit(_ unit: String) {
        ...
        tempUnit = unit
        saveConfigState()
        // Push the choice to the device too (firmware UNIT handler accepts
        // "C"/"F" and persists it), so the OLED and the app always agree.
        // Previously this was display-local only, and the device's unit
        // notification would silently override the user's pick here.
        // Harmless no-op while disconnected.
        writeCharacteristic(uuidUnit, value: unit)
    }
    ...
    // NOTE: no centralManager(_:willRestoreState:) here on purpose — the central
    // is created WITHOUT a restore identifier (see init), so CoreBluetooth never
    // calls it. attemptRestoreSavedPeripheral() below is this app's reconnect path.
    ...
    private func updateStatusFromCharacteristic(_ uuid: CBUUID, data: Data) {
        ...
        case uuidBat:
            // Firmware sends an integer percent (BLEManager.cpp formats "%d").
            if let pct = Int(s) {
                status.batPct = pct
            } else {
        ...
        case uuidKp:
            if let v = Double(s) { kp = v }

        case uuidKi:
            if let v = Double(s) { ki = v }

        case uuidKd:
            if let v = Double(s) { kd = v }
    ...
    /// Set the (Auto-loop) setpoint: writes the setpoint characteristic,
    /// optimistically mirrors the value locally, and persists it.
    ...
    /// Write the current kp/ki/kd to the device and persist them.
    func savePID() {
        setPID(kp: kp, ki: ki, kd: kd)
        saveConfigState()
    }

    /// Reset PID to the shipped defaults, write to the device, and persist.
    func resetPID() {
        let def = Self.defaultPID
    ...
    /// Set mode. Sends the numeric mode index as a plain decimal string on the
    /// Mode characteristic, matching the firmware's onWrite handler
    /// (BLEManager.cpp), which parses it with atoi() and calls applyPidMode().
    /// The app only ever sends Auto (0) or Manual (1) — U1/U2/Config are chosen
    /// on the device dial, and the app just displays them when reported.
