...
struct ContentView: View {
            ...
            ScrollView { // SINGLE global scroll view
                VStack(spacing: Theme.sBlock) {
                    ...
                    VStack(alignment: .leading, spacing: Theme.sCard) {
                        ...
                        Text("Target source: \(viewModel.status.spmode ? "Device dial" : "App")")
                            .statLabel()
                    }
                    .card()
    ...
    // Save a script/note to a profile on-device. The firmware persists the payload to NVS
    // (see UUID_SAVE_SCRIPT in ble.h / docs/BLE-PROTOCOL.md). Currently unused by the UI —
    // wire a control to it in the "UI polish" roadmap phase.
    private func saveScriptToProfile(profileName: String, script: String) {
        // Firmware command format: "SAVE:<profile>:<payload>" (profile = AUTO | U1 | U2).
        let payload = "SAVE:\(profileName):\(script)"
        // writeCharacteristic handles UTF-8 encoding and missing-characteristic errors itself.
...
// live in the Settings sheet. modeIndex is the device mode it writes (0 = Auto).
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Theme.sBlock) {
            ...
            VStack(alignment: .leading, spacing: Theme.sRow) {
                ...
                .tint(Theme.seriesColor(viewModel.chartColorIndex))
                .disabled(viewModel.pidMode == 1 || !viewModel.isConnected) // PID mode owns output; no connection means no write
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: Theme.sBlock) {
            ...
            VStack(alignment: .leading, spacing: Theme.sRow) {
                ...
                infoRow("Firmware", viewModel.firmwareVersion)
                NumberField(label: "Default target (°C)", text: $defaultSetpointString)
                    .onChange(of: defaultSetpointString) { _, val in
                        // Same ceiling as the slider: firmware faults above 320 °C.
                        if let d = Double(val) { viewModel.defaultSetpoint = min(max(d, 165), 315) }
                    }
        ...
        .frame(maxWidth: .infinity)
        .onAppear {
            defaultSetpointString = String(format: "%.1f", viewModel.defaultSetpoint)
        }
    }
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: Theme.sBlock) {
                    ...
                    // PID tuning (Auto mode, index 0).
                    VStack(alignment: .leading, spacing: Theme.sCard) {
                        PIDTuningView(viewModel: viewModel, kpString: $kpString,
                                      kiString: $kiString, kdString: $kdString, modeIndex: 0)
                    }
                    .card()
