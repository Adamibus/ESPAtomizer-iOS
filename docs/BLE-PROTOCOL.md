...
- **Range**: 100.0 - 400.0 °C (`ENC_MIN_C`..`ENC_MAX_C`, encoder.h)
...
- **Persistence**: None — the live setpoint is session-scoped (hold-persists).
  A cold boot starts from the **Default Setpoint** preset; a deep-sleep wake
  restores the last live value from RTC memory.
- **Notify**: The device pushes device-side changes (dial turns, preset loads)
  so the app tracks them; app-originated writes are not echoed back.
- **Example**:
  ```
...
- **Valid Values**:
  - `"0"` = Auto (PID active; the live setpoint is kept — the default-setpoint preset applies only at cold boot)
  - `"1"` = Manual (fixed output)
  - `"2"` = U1 preset
  - `"3"` = U2 preset
  - `"4"` = Config
- **Response**: Device calls `applyPidMode(n)` and echoes the new mode on **Mode Read** (`…8002`).
- **⚠ Not** `"AUTO"/"MAN"/…` — those are rejected (`atoi` reads them as `0`).
- **Example**:
  ```
  Write: "0"   → Auto mode
  Write: "1"   → Manual mode
...
- **Properties**: READ, WRITE, NOTIFY (live duty published ~1 Hz while it changes)
...
#### 10. **Status / Write Ack** (Read-Only, Notifiable)
...
- **Behavior**: Notified after **every** processed write so the client can tell "applied" from
  "silently rejected". `<FIELD>` ∈ `EN, SP, KP, KI, KD, DSP, UNIT, OUT, MODE`;
  `<reason>` ∈ `range, fmt, empty`. Initial value `OK:INIT`.
- **Example**: setpoint `"999"` (out of range) → notify `ERR:SP:range`

---

#### 11. **Protocol Version** (Read-Only)
...
|----------------|-----------|-----------------|
| Setpoint | Range 100-400°C | `[BLE] WARNING: Rejected invalid setpoint` |
...
| Name | UUID | Purpose |
|------|------|---------|
| Enable | `3f1a0001-2a8d-4a54-8f2f-b7cd2b4b8001` | Power on/off (RW, Notify) |
| Setpoint | `3f1a0002-2a8d-4a54-8f2f-b7cd2b4b8001` | Target temperature (RW, Notify) |
...
| Output | `3f1a0008-2a8d-4a54-8f2f-b7cd2b4b8001` | PWM output (RW, Notify) |
