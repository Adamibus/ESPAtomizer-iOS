...
void PreferencesManager::setSetpoint(double valueC, int mode) {
  // ENC_MIN_C and ENC_MAX_C defined in config.h
  #ifndef ENC_MIN_C
  ...
  if (mode == 2) {
    prefs.putString("u1_sp", buf);
    Serial.printf("[PREF] U1 Setpoint saved: %.1fC\n", valueC);
  } else if (mode == 3) {
    prefs.putString("u2_sp", buf);
    Serial.printf("[PREF] U2 Setpoint saved: %.1fC\n", valueC);
  } else {
    prefs.putString("sp", buf);
    Serial.printf("[PREF] Setpoint saved: %.1fC\n", valueC);
  }
  
  prefs.end();
}

double PreferencesManager::getSetpoint(int mode) {
  Preferences prefs;
  prefs.begin(PREF_NAMESPACE, true);
  
  const char* key = (mode == 2) ? "u1_sp" : (mode == 3) ? "u2_sp" : "sp";
  String v = prefs.getString(key, String());
  ...
  if (mode == 2) return 220.0;  // U1 default
  if (mode == 3) return 180.0;  // U2 default
  return 200.0;                  // AUTO default
}
