...
void setup() {
  ...
  // Apply the device preset as the boot target (hold-persists semantics: this
  // is the ONE place the preset becomes the live setpoint). On a deep-sleep
  // wake the RTC restore below overwrites this with the last live target, so
  // the preset only takes effect on a true cold boot — and applyPidMode(0) no
  // longer re-applies it on every Auto entry.
  gState.pidController.setpointC = constrain(gState.pidController.defaultSetpoint,
                                             (double)ENC_MIN_C, (double)ENC_MAX_C);
  // Restore saved PID tunings from NVS. The BLE handlers have always persisted
  // these on every accepted write, but nothing ever read them back — so "Save"
  // from the app silently did not survive a cold boot. Clamped to the same
  // ranges the BLE write handlers enforce. (Deep-sleep wake: the RTC restore
  // below overrides with the last live tunings, same as the setpoint.)
  gState.pidController.Kp = constrain(PreferencesManager::getKp(), 0.1, 100.0);
  gState.pidController.Ki = constrain(PreferencesManager::getKi(), 0.01, 10.0);
  gState.pidController.Kd = constrain(PreferencesManager::getKd(), 0.1, 1000.0);
...
#if USE_BLE
  // Now that preferences + RTC state are restored, seed every readable BLE
  // characteristic's value. BLEManager::init() ran BEFORE the restore, so
  // without this the app's connect-time reads returned empty strings and its
  // UI silently kept stale local values instead of the device's real state.
  BLEManager::seedCharacteristicValues();
#endif
...
void loop() {
  ...
  // Check battery safety: charger removal, voltage instability, low warning
    ...
    didRead = true;
      ...
      // Push device-side state changes (power / setpoint / output) to the app.
      // gState.ble.pub* are also updated by the BLE write callback on every
      // accepted app write, so app-originated changes are never echoed back —
      // a late echo can race a slider mid-drag and snap it backwards. Only
      // changes made HERE (dial, presets, safety shutoff, PID loop) notify.
      if (bleIsConnected()) {
        // Power: immediately on change. The 30-min safety shutoff, cooldown
        // re-disable and the encoder button all flip this without BLE's
        // knowledge — the app previously showed Power On after a safety cut.
        bool en = gState.pidController.systemEnabled;
        if ((!gState.ble.pubEnableSeeded || en != gState.ble.pubEnable) && gState.ble.chEnable) {
          gState.ble.chEnable->setValue(en ? "1" : "0");
          gState.ble.chEnable->notify();
          gState.ble.pubEnable = en;
          gState.ble.pubEnableSeeded = true;
        }
        // Setpoint: immediately on change (dial turns, preset loads).
        double sp = gState.pidController.setpointC;
        if ((isnan(gState.ble.pubSetpoint) || fabs(sp - gState.ble.pubSetpoint) >= 0.05) && gState.ble.chSetpoint) {
          char sbuf[16];
          snprintf(sbuf, sizeof(sbuf), "%.1f", sp);
          gState.ble.chSetpoint->setValue(std::string(sbuf));
          gState.ble.chSetpoint->notify();
          gState.ble.pubSetpoint = sp;
        }
        // Output: ~1 Hz, on change. The PID loop moves this constantly in
        // Auto; the app's Output stat and output strip chart were dead
        // because nothing ever published the live duty cycle.
        static unsigned long lastOutBleMs = 0;
        if ((now - lastOutBleMs) >= 1000UL) {
          lastOutBleMs = now;
          int o = (int)gState.pidController.pidOutput;
          if (o != gState.ble.pubOut && gState.ble.chOut) {
            char obuf[12];
            snprintf(obuf, sizeof(obuf), "%d", o);
            gState.ble.chOut->setValue(std::string(obuf));
            gState.ble.chOut->notify();
            gState.ble.pubOut = o;
          }
        }
      }
...
void applyPidMode(int mode) {
  ...
  if (mode == 0) {
    // AUTO: keep current tunings, ensure automatic. Hold-persists: the live
    // setpoint deliberately survives mode changes — re-entering Auto no longer
    // resets it to the device preset (defaultSetpoint). The preset is applied
    // ONCE, at cold boot in setup(); after that the dial/app own the target.
